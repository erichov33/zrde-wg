...
exports.workflowService = exports.UnifiedWorkflowService = void 0;
class UnifiedWorkflowService {
    ...
    async executeWorkflowDirect(workflow, context) {
    ...
    evaluateBusinessRule(rule, context) {
    ...
    executeCalculation(calculation, context) {
        ...
        switch (operation) {
            ...
            case "subtract":
                return values.reduce((diff, val) => diff - val);
            case "multiply":
    ...
    async executeDataSource(node, context) {
    ...
    async executeAIDecision(node, context) {
        // Placeholder for AI decision logic
        return { ...context.data, aiDecision: "ai_result" };
    }
    async executeBatchProcess(node, context) {
    ...
    async executeAuditLog(node, context) {
        // Placeholder for audit logging
        return { ...context.data, auditLogged: true };
    }
    getNextNode(currentNode, workflow, context, nodeResult) {
    ...
    evaluateConnectionCondition(connection, context, nodeResult) {
    ...
    // ==================== Utility Methods ====================
    createDefaultWorkflow(mode = "simple") {
    ...
    addExecutionHistory(workflowId, result) {
    ...
    async batchExecuteWorkflows(requests) {
        const results = await Promise.allSettled(requests.map(req => this.executeWorkflow(req.workflowId, req.context)));
    ...
    async batchValidateWorkflows(workflowIds) {
        const workflows = await Promise.all(workflowIds.map(async (id) => ({ id, workflow: await this.getWorkflow(id) })));
        const validations = await Promise.all(workflows.map(async ({ id, workflow }) => ({
            id,
...
exports.UnifiedWorkflowService = UnifiedWorkflowService;
// Export singleton instance
exports.workflowService = new UnifiedWorkflowService();
